...
function SchemaTable<T, F>({
  ...
  const handleChangePage = useCallback(
    (event: React.MouseEvent<HTMLButtonElement> | null, newPage: number) => {
      setConfig((c) => ({ ...c, pageNumber: newPage }));
    },
    []
  );
  ...
  const handleRequestSort = useCallback(
    (event: React.MouseEvent<unknown>, property: string) => {
      setConfig((c) => {
        const isDesc = c.orderBy === property && c.order === "desc";
  ...
  const handleOnFilter = useCallback((obj?: F) => {
    setConfig((c) => ({ ...c, pageNumber: 0, filter: obj }));
  }, []);
  ...
  const startEdit = useCallback(
    async (state: SchemaFormStates, row?: T) => {
      if (row && onChange) {
  ...
  const handleOnEditSaveSuccess = useCallback(
    (row: T) => {
      if (editState === "ADD") {
  ...
  const handleOnFormChange = useCallback(
    (obj: T, changeObj: Partial<T>): void => {
      if (onChange) {
        const asyncNewSchema = onChange(
          schema as FormSchema<T>,
          obj,
          changeObj
        );
