...
import {
  ExpenseSchemaContext,
  ExpenseFilter,
  ExpenseSchemaContextProvider,
  ExpensesTableConfig,
} from "./ExpensesSchemaContext";
...
import CoreTableHead, {
  HeadRow,
} from "../core/components/tables/CoreTableHead";
...
function FinanceExpenses() {
  ...
  const [schema, setSchema] = useState<FormSchema<Expense>>(() =>
    schemaContext.get({ type: "ADD" })
  );
  const [filterSchema, setFilterSchema] = useState<FormSchema<ExpenseFilter>>(
    () => schemaContext.get({ type: "FILTER" })
  );
  const [config, setConfig] = React.useState<ExpensesTableConfig>({
    ...schemaTableConfig,
    filter: schemaContext.get<ExpenseFilter>({ type: "FILTER" }).object,
  });
  const [expenseSummary, setExpenseSummary] = React.useState<ExpenseSummary>({
    expenses: [],
    plannedAmount: 0,
    totalActualAmount: 0,
    remainder: 0,
  ...
  function handleRequestSort(
    ...
    var expenses = [...(expenseSummary.expenses || [])].sort(
      getSorting(newOrder, property)
    );
    ...
    <Fragment>
      <div className={classes.total}>
        ...
        <span>
          ...
          <span
            className={clsx(
              (expenseSummary.remainder || 0) < 0
                ? classes.positive
                : classes.negative
            )}
          >
            {currencyFormatter.format(expenseSummary.remainder || 0)}
          </span>
        </span>
      </div>
          ...
          <TableBody>
            {expenseSummary?.expenses?.map((row, idx) => (
              ...
              >
                ...
                <TableCell>
                  {currencyFormatter.format(row.plannedAmount || 0)}
                </TableCell>
                <TableCell>
                  {currencyFormatter.format(row.actualAmount || 0)}
                </TableCell>
                <TableCell
                  className={clsx(
                    (row.remainder || 0) < 0
                ...
                >
                  {currencyFormatter.format(row.remainder || 0)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
