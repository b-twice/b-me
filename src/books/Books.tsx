...
function Books() {
  ...
  const [filterSchema, setFilterSchema] = useState<FormSchema<BookFilter>>(() =>
  ...
  const [config, setConfig] = React.useState<BooksTableConfig>({
    ...schemaTableConfig,
    filter: schemaContext.get<BookFilter>({ type: "FILTER" }).object,
  });
